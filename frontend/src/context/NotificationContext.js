...
export function NotificationProvider({ children }) {
  ...
  const alertForNotification = useCallback(
    (newNotif) => {
      // Suppress duplicate native/system alerts for the same event.
      const key = alertEventSignature(newNotif);
      const now = Date.now();
      ...
      // Ignore the same event if it was already alerted within 15 seconds.
      if (recentlyAlertedRef.current.has(key)) return;
      recentlyAlertedRef.current.set(key, now + 15000);
